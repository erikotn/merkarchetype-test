 ...
 */
function voegCombinatiesToe() {
  const ui = SpreadsheetApp.getUi();
  const ss = SpreadsheetApp.getActiveSpreadsheet();
  const newRows = JSON.parse(COMBINATIONS_SEED);
  const dataRows = newRows.slice(1); // skip header

  let sheet = ss.getSheetByName(COMBINATIONS_TAB);
  if (!sheet) {
    populateSheet(ss, COMBINATIONS_TAB, newRows);
    sheet = ss.getSheetByName(COMBINATIONS_TAB);
    sheet.setColumnWidth(3, 600);
    ui.alert('Klaar', dataRows.length + ' combinatie-teksten toegevoegd in een nieuwe Combinations-tab.', ui.ButtonSet.OK);
    return;
  }

  // Tab bestaat — verzamel bestaande paren bidirectioneel
  const existing = new Set();
  const existingData = sheet.getDataRange().getValues();
  for (let i = 1; i < existingData.length; i++) {
    const p = String(existingData[i][0] || '').trim();
    const s = String(existingData[i][1] || '').trim();
    if (p && s) {
      existing.add(p + '|' + s);
      existing.add(s + '|' + p);
    }
  ...
  const toAppend = dataRows.filter(r => {
    const p = String(r[0]).trim();
    const s = String(r[1]).trim();
    return !existing.has(p + '|' + s);
  });

  if (toAppend.length === 0) {
    ui.alert('Niets toegevoegd', 'Alle combinaties stonden al in de tab.', ui.ButtonSet.OK);
    return;
  }
  ...
  ui.alert(
    'Klaar',
    toAppend.length + ' nieuwe combinatie-teksten toegevoegd aan Combinations-tab.\n\n' +
    'Bestaande rijen ongemoeid gelaten. Bewerk gerust in de Sheet — de tool leest live.',
    ui.ButtonSet.OK
  );
}
...
const COMBINATIONS_SEED = '[["primary", "secondary", "description"], ["caregiver", "creator", "Zorg gepaard met creatie. Jullie maken iets waardevols én letten op wie er gebruik van maakt. De Beschermer geeft de Creator een kompas: niet maken om te maken, maar maken voor mensen. De spanning: de Creator wil grenzen verleggen, de Beschermer wil schade voorkomen. Goede merken in dit kwadrant maken zorgvuldige producten met een ziel."], ["caregiver", "innocent", "Een warme, betrouwbare combinatie. De Onschuldige gelooft dat het goed komt, de Beschermer zorgt dat het ook gebeurt. Klanten ervaren dit als veilig haven-merk: vriendelijk, toegankelijk, zonder verborgen agenda. Wees alert op suikerzoetheid — als beide archetypes té dominant zijn wordt het brave brand. Voeg ergens een scherpere kant toe (humor, expertise, ambitie) om relevant te blijven."], ["caregiver", "regular", "Het merk dat dichtbij staat én voor je opkomt. Geen pretentie, geen hoge toon — gewoon iemand die snapt waar jij doorheen gaat en bereid is te helpen. De Bondgenoot zorgt voor herkenning, de Beschermer voor toewijding. In de B2B-wereld werkt dit goed voor diensten waar vertrouwen langer duurt dan een transactie. Risico: onzichtbaar zijn omdat je nooit op de borst klopt."], ["caregiver", "sage", "De wijze beschermer. Klanten komen voor de expertise en blijven voor de zorg. Veel succesvolle gezondheids-, verzekerings- en kennisinstituten leven hier: Zilveren Kruis, ANBO, ouderwetse familiedokter. De Wijze geeft autoriteit, de Beschermer geeft gevoel. Maak je niet schuldig aan moedig betuttelen — de combinatie fluistert het beste, schreeuwt nooit."], ["creator", "jester", "Speelse vindingrijkheid. De Joker maakt de Creator licht: eindeloze experimenteer-modus, fouten zijn part of the deal. Verhalen krijgen een knipoog, producten een glimlach. Coolblue, Lego en Old Spice draaien op deze as. Spanning: blijf het ambacht serieus nemen ook als de communicatie speels is — anders wordt het een gimmick zonder substantie."], ["creator", "lover", "Maken met liefde voor schoonheid. De Verleider zorgt dat de Creator niet alleen functioneel is, maar ook iets dat raakt. Esthetiek als prioriteit, geen toevoeging. Apple, Rituals en Marimekko leven hier. Hoed je voor zelfingenomenheid: een merk dat te veel naar de spiegel kijkt verliest contact met de gebruiker. Schoonheid is alleen relevant als ze iets dient."], ["creator", "magician", "Visionaire maakkracht. De Magiër droomt het ondenkbare, de Creator brengt het tot leven. Tesla, Studio Job, en de meeste echte design-pioniers zitten hier. Klanten kopen niet alleen een product — ze kopen een toekomstbeeld. Het gevaar: zo zeer voor jezelf bouwen dat de markt \'m niet kan volgen. Een Magiër-Creator merk heeft een vertaler nodig (jouw communicatie of een evangelist)."], ["creator", "sage", "Doordachte vakmanschap. De Wijze onderzoekt, de Creator bouwt. Geen impulsief experiment — elke designkeuze is onderbouwd. Adobe, Bose, Bauhaus-school: alles waar het \'waarom\' net zo zwaar weegt als het \'wat\'. De spanning zit in tempo: de Wijze wil eerst dieper graven, de Creator wil al maken. Ritueel om beide te eren: prototype eerst, dan onderzoek waarom \'t werkt."], ["explorer", "creator", "Ontdekken door te maken. Niet eerst onderzoeken, dan bouwen — maar bouwend onderzoeken. Studio\'s, makerslabs en innovatieve bureaus leven hier. De Ontdekker biedt de moed om ergens onbekend te beginnen, de Creator de discipline om af te maken. Voor klanten voelt dit als \'een merk dat doet wat ik zelf zou willen doen, als ik durfde\'."], ["explorer", "hero", "De moedige avonturier. Niet alleen op pad, maar ook door obstakels heen. KLM in haar gloriedagen, The North Face, Patagonia: merken die tonen dat avontuur ook overwinnen betekent. De Held maakt de Ontdekker doelgericht. De Ontdekker houdt de Held nieuwsgierig. Communicatie balanceert tussen wijde uitzichten en zweet op het voorhoofd."], ["explorer", "magician", "Reizen die transformeren. Niet alleen fysiek ergens komen, maar als ander mens terug. Eat Pray Love-merken, retraite-aanbieders, Lonely Planet voor de gedreven traveler. De Magiër belooft verandering, de Ontdekker biedt het pad. Vermijd vaagheid — als de transformatie te etherisch wordt verdwijnt geloofwaardigheid. Concrete rituelen of momenten als ankerpunten."], ["explorer", "outlaw", "Ongebaande paden, regelovertredend. Harley-Davidson, Patagonia in haar activistische modus, sommige outdoor-merken die natuurbescherming agressief op de agenda zetten. De Rebel scherpt de Ontdekker: avontuur niet als ontspanning, maar als statement. Risico: te veel onstuimigheid, te weinig richting. Een doel hebben (zelfontdekking, klimaatactie) bindt de energie."], ["hero", "magician", "Doelgerichte transformatie. De Magiër droomt, de Held doet — samen veranderen ze de wereld. Tesla, Greenpeace, missiegedreven scale-ups. Klanten ervaren dit als merken die hen niet alleen inspireren maar ook iets concreets bieden om aan te dragen. Pas op voor messianisme — als jullie te veel gaan geloven in eigen heiligheid, raken jullie de mens op straat kwijt."], ["hero", "ruler", "De top-presteerder. Mercedes-AMG, Goldman Sachs, Champions League-clubs. Niet zomaar winnen, ook regeren. De Held maakt de Leider sportief, de Leider geeft de Held gewicht. In B2B werkt dit voor merken die premium prijzen rechtvaardigen met aantoonbare excellence. Risico: zelfgenoegzaamheid. De moment dat je gelooft dat je het verdient om te winnen, ben je ingehaald."], ["hero", "sage", "De doortastende expert. Klanten zien jullie als gids die niet alleen kennis heeft maar ook durft te handelen. McKinsey op een goede dag, een topadvocaat, een chirurg. De spanning zit in tempo: de Wijze wil eerst onderzoeken, de Held wil direct in actie. Een goed merk in dit kwadrant communiceert kalm én urgent — het verschil tussen advies en regie."], ["innocent", "jester", "Kinderlijke vrolijkheid. Niets achter zoeken, gewoon van het moment genieten. M&M\'s, Skittles, Nutella in haar vroege jaren. De Onschuldige geeft pure intentie, de Joker geeft energie. Merken hier voelen vriendelijk, nooit agressief. Risico: oppervlakkigheid. Voeg ergens een diepere laag toe (familietraditie, ambacht) zodat je ook met serieuzere momenten kunt landen."], ["innocent", "regular", "Eenvoud die werkt. HEMA, Friesche Vlag, basic boodschappen. De Bondgenoot geeft toegankelijkheid, de Onschuldige geeft schoonheid in eenvoud. Klanten weten precies wat ze krijgen — geen verrassingen, geen hoge toon. In een wereld van marketing-overload is dit een welkom rustpunt. Zorg dat \'simpel\' niet \'simpelistisch\' wordt — er is verschil tussen elegante en banale eenvoud."], ["innocent", "sage", "Heldere wijsheid. De Wijze maakt het concept toegankelijk via de Onschuldige. NUS, Klokhuis, Wikipedia in haar essentie. Geen jargon, geen ego — gewoon delen wat je weet. Klanten ervaren dit als \'eindelijk iemand die het uitlegt zonder me te kleineren\'. De spanning: te veel vereenvoudigen verliest de nuance. Verteller zijn, niet versimpelaar."], ["jester", "lover", "Speelse zinnelijkheid. Het leven is een feest, en dat feest moet je proeven. Caféketens, sommige hotels, Aperol Spritz. De Verleider geeft sensueel genot, de Joker geeft luchtigheid. Klanten voelen: dit is een merk dat me het plezier gunt. Risico: vluchtigheid. Een Joker-Verleider merk heeft moeite met hardere momenten — bouw ergens een serieuze ondertoon in (kwaliteitstrouw, ritueel)."], ["jester", "regular", "Herkenbare humor. Coolblue in optima forma — gewone mensen die elkaar aan het lachen maken. Geen ironie van bovenaf, geen elite-grappen. De Bondgenoot zorgt dat de humor inclusief blijft, de Joker zorgt dat het merk geen zeurpiet wordt. Sterk merk-archetype voor consumenten-services. Hoed je voor inflatie: als alles een grap moet zijn, gelooft niemand de serieuze boodschappen meer."], ["jester", "sage", "Geestige intelligentie. Denken met humor — de combinatie van een Toon Hermans-college. The Daily Show, sommige Britse weekendkranten. De Wijze geeft inhoud, de Joker zorgt dat de inhoud niet in eigen voet schiet. Voor B2B een verfrissend alternatief: experts die zichzelf relativeren, vertrouwen winnen door eerlijkheid in plaats van plechtstatigheid."], ["lover", "magician", "Transformatieve schoonheid. Rituals in haar essentie, Chanel No. 5, retraite-merken. De Magiër belooft verandering, de Verleider verleidt naar het ritueel toe. Klanten kopen niet een product, maar een momentje van veranderen. Risico: pretentie. Een Verleider-Magiër merk dat te veel betekenis claimt, klinkt als een spa-folder. Houd het zintuiglijk én bescheiden."], ["lover", "ruler", "Statige zinnelijkheid. Luxe modehuizen, Hermès, sommige autocouture. De Leider geeft prestige, de Verleider geeft begeerte. Klanten ervaren dit als \'iets dat ik mezelf gun, omdat ik het verdien\'. Het werkt alleen als de prijspositie matcht — een Verleider-Leider merk in de mid-market voelt forced. Schaarste, ambacht en ritueel zijn natuurlijke ankers."], ["magician", "ruler", "Visionaire autoriteit. Steve Jobs-tijdperk Apple, Microsoft onder Satya Nadella, sommige consultancybureaus die echt sturen. De Magiër ziet wat anderen niet zien, de Leider zorgt dat het ook gebeurt. Klanten geven over: \'jullie weten waar het heen gaat\'. Risico: arrogantie. Visie wordt pretentie zodra je niet meer luistert. Houdt openheid voor andere stemmen formeel ingebouwd."], ["magician", "sage", "Wijze visionair. Niet alleen een toekomstbeeld, ook de onderbouwing. Universiteiten in optima forma, voorlopers in tech (DeepMind), strategische denktanks. De Wijze houdt de Magiër verantwoordelijk, de Magiër houdt de Wijze nieuwsgierig. Krachtig in B2B: experts die niet alleen weten maar ook richten. Communicatie balanceert tussen feit en verbeelding."], ["outlaw", "creator", "Disruptieve creatie. Niet meedoen met de bestaande regels — eigen taal, eigen vorm, eigen weg. Tony\'s Chocolonely, Patagonia in activistische modus, vroege Vice. De Rebel scherpt de Creator: niet maken voor het maken, maken om iets te kantelen. Krachtig voor missie-gedreven merken. Hoed je voor pose: rebellie zonder substantie verbleekt snel."], ["outlaw", "jester", "Brutale humor. Ben & Jerry\'s, vroege MTV, Comedy Central. De Joker maakt de Rebel landbaar, de Rebel zorgt dat de humor bijt. Klanten waarderen merken die durven te lachen om wat anderen niet durven aan te raken. Risico: schoffeerderigheid. Het verschil tussen scherpe humor en brutaal voor de show is razendsmal."], ["outlaw", "magician", "Disruptieve transformatie. Tesla in haar early-adopter-fase, Elon Musk-merken in algemene zin, baanbrekende startups. De Rebel breekt het bestaande af, de Magiër bouwt het nieuwe op. Krachtig combinatie als jullie de wereld echt willen kantelen. Risico: te ver vooruit lopen. Klanten moeten kunnen meegroeien — anders ben je een eenmansparade."], ["regular", "sage", "Toegankelijke expertise. Kennis zonder kapsones. Wikipedia, een bijzondere apotheker, Volkskrant in haar essentie. De Wijze maakt de Bondgenoot scherp, de Bondgenoot maakt de Wijze ontvankelijk. Klanten voelen: \'eindelijk iemand die het uitlegt op mijn niveau, zonder me te kleineren\'. Krachtig voor merken die complexe materie democratiseren. Vermijd populisme: helderheid is iets anders dan oppervlakkigheid."], ["ruler", "sage", "Vertrouwde autoriteit. Banken in hun beste vorm (ABN AMRO, ING vroeger), gerenommeerde universiteiten, instituten zoals het CPB. De Wijze geeft inhoudelijk gezag, de Leider geeft positionele zwaarte. Klanten ervaren: \'wat zij zeggen klopt, en het wordt ook gehoord\'. Risico: gebureaucratiseerd raken. Houd ergens een directe stem — anders verzinkt het merk in protocollen."]]';
