...
function readCombinations() {
  ...
  if (!rows) return null;
  // Naar nested object: { primary: { secondary: { description, type, tone, dos, donts, example } } }
  // ToV-velden (tone/dos/donts/example) zijn optioneel — komen alleen mee als de kolommen bestaan.
  const out = {};
  rows.forEach(r => {
    ...
    if (!p || !s || !d) return;
    if (!out[p]) out[p] = {};
    const entry = { description: d, type: t };
    // Optionele ToV-velden
    const tone = String(r.tone || '').trim();
    const dos = String(r.dos || '').trim();
    const donts = String(r.donts || '').trim();
    const example = String(r.example || '').trim();
    if (tone || dos || donts || example) {
      entry.tone = tone;
      entry.dos = dos;
      entry.donts = donts;
...
/**
 * Verrijkt de bestaande Combinations-tab met 4 ToV-kolommen:
 * tone, dos, donts, example. 132 voorgeschreven entries worden ingevuld
 * op basis van (primary, secondary) matching.
 *
 * Werkt non-destructief:
 * - Combinations-tab moet bestaan met minstens primary + secondary kolommen
 *   (anders: vraag eerst vervangAlleCombinaties te runnen)
 * - Bestaande ToV-data wordt overschreven na bevestiging
 * - Geen aanraking van de description-kolom of andere data
 */
function voegToneOfVoiceToe() {
  const ui = SpreadsheetApp.getUi();
  const ss = SpreadsheetApp.getActiveSpreadsheet();
  const sheet = ss.getSheetByName(COMBINATIONS_TAB);

  if (!sheet) {
    ui.alert(
      'Combinations-tab ontbreekt',
      'Run eerst vervangAlleCombinaties om de Combinations-tab te vullen.',
      ui.ButtonSet.OK
    );
    return;
  }
  ...
  const data = sheet.getDataRange().getValues();
  if (data.length < 2) {
    ui.alert('Combinations-tab is leeg', 'Run eerst vervangAlleCombinaties.', ui.ButtonSet.OK);
    return;
  }

  // Vind kolommen primary, secondary; bepaal of tone/dos/donts/example al bestaan
  const headers = data[0].map(h => String(h).trim().toLowerCase());
  const colPrimary = headers.indexOf('primary');
  const colSecondary = headers.indexOf('secondary');
  if (colPrimary === -1 || colSecondary === -1) {
    ui.alert('Kolom ontbreekt', 'De Combinations-tab moet primary en secondary kolommen hebben.', ui.ButtonSet.OK);
    return;
  }

  // Detecteer of de ToV-kolommen al bestaan
  let colTone = headers.indexOf('tone');
  let colDos = headers.indexOf('dos');
  let colDonts = headers.indexOf('donts');
  let colExample = headers.indexOf('example');
  const tovColsExist = colTone !== -1 && colDos !== -1 && colDonts !== -1 && colExample !== -1;

  if (tovColsExist) {
    const resp = ui.alert(
      'ToV-kolommen bestaan al',
      'De kolommen tone, dos, donts en example bestaan al. Overschrijven met de 132 voorgeschreven teksten?\n\nEigen aanpassingen in deze kolommen gaan verloren.',
      ui.ButtonSet.YES_NO
    );
    if (resp !== ui.Button.YES) return;
  } else {
    // Voeg ontbrekende kolommen toe aan het einde
    const startCol = headers.length + 1;
    const newHeaders = [];
    if (colTone === -1) { newHeaders.push('tone'); colTone = headers.length; headers.push('tone'); }
    if (colDos === -1)  { newHeaders.push('dos');  colDos = headers.length;  headers.push('dos'); }
    if (colDonts === -1){ newHeaders.push('donts');colDonts = headers.length;headers.push('donts'); }
    if (colExample === -1){newHeaders.push('example'); colExample = headers.length; headers.push('example'); }
    sheet.getRange(1, startCol, 1, newHeaders.length).setValues([newHeaders])
      .setFontWeight('bold').setBackground('#f3f3f3');
  }

  // Bouw lookup: { 'primary|secondary': [tone, dos, donts, example] }
  const lookup = {};
  TONE_OF_VOICE_DATA.slice(1).forEach(row => {
    lookup[row[0] + '|' + row[1]] = [row[2], row[3], row[4], row[5]];
  });

  // Loop over de rijen in de Sheet en schrijf ToV per paar
  let filled = 0, missing = 0;
  for (let i = 1; i < data.length; i++) {
    const p = String(data[i][colPrimary] || '').trim();
    const s = String(data[i][colSecondary] || '').trim();
    if (!p || !s) continue;
    const tov = lookup[p + '|' + s];
    if (!tov) { missing++; continue; }
    sheet.getRange(i + 1, colTone + 1).setValue(tov[0]);
    sheet.getRange(i + 1, colDos + 1).setValue(tov[1]);
    sheet.getRange(i + 1, colDonts + 1).setValue(tov[2]);
    sheet.getRange(i + 1, colExample + 1).setValue(tov[3]);
    filled++;
  }

  // Kolombreedtes
  sheet.setColumnWidth(colTone + 1, 400);
  sheet.setColumnWidth(colDos + 1, 500);
  sheet.setColumnWidth(colDonts + 1, 500);
  sheet.setColumnWidth(colExample + 1, 400);

  let msg = filled + ' rij' + (filled === 1 ? '' : 'en') + ' verrijkt met Tone of Voice-data.';
  if (missing > 0) msg += '\n\n' + missing + ' paren niet gevonden in TONE_OF_VOICE_DATA — controleer of primary/secondary keys kloppen.';
  ui.alert('Klaar', msg, ui.ButtonSet.OK);
}
...
const TONE_OF_VOICE_DATA = [
  [`primary`, `secondary`, `tone`, `dos`, `donts`, `example`],
  [`innocent`, `sage`, `Helder, hoopvol én onderbouwd. We geloven in het goede en checken eerst de feiten. Een toon die kalmeert zonder oppervlakkig te worden.`, `Spreek in korte, alledaagse zinnen | Houd de boodschap positief en concreet | Onderbouw uitspraken met feit of bron | Leg de redenering uit, niet alleen de conclusie`, `Vermijd cynisme of ironie | Maak het niet zwaar of dramatisch | Vermijd jargon dat alleen experts begrijpen | Niet uit de hoogte spreken of betuttelen`, `Het zit eenvoudiger dan je denkt — en de cijfers laten zien waarom.`],
  [`innocent`, `explorer`, `Open en hoopvol, met een uitnodiging om mee op pad te gaan. We staan voor het goede én voor wat er nog te ontdekken valt.`, `Spreek in korte, alledaagse zinnen | Houd de boodschap positief en concreet | Nodig uit, in plaats van overtuigen | Spreek over mogelijkheden en open horizonten`, `Vermijd cynisme of ironie | Maak het niet zwaar of dramatisch | Vermijd voorschrijven of geboden | Geen kleinmakende boodschap`, `Stap voor stap kom je verder dan gedacht — en we lopen graag een eindje mee.`],
  [`innocent`, `hero`, `Eerlijk, optimistisch en doelgericht. We geloven dat het goed komt — en gaan ervoor.`, `Spreek in korte, alledaagse zinnen | Houd de boodschap positief en concreet | Spreek in actie-werkwoorden | Wees concreet over wat we gaan doen`, `Vermijd cynisme of ironie | Maak het niet zwaar of dramatisch | Vermijd onnodige slagen om de arm | Geen marketing-bombarie of holle claims`, `Het kan, en we beginnen vandaag.`],
  [`innocent`, `outlaw`, `Hoopvol, maar bereid de regels te bevragen. We zien het goede in mensen en weigeren te wachten op wie ons in de weg staat.`, `Spreek in korte, alledaagse zinnen | Houd de boodschap positief en concreet | Spreek scherp en zonder omhaal | Kies positie, ook als het ongemakkelijk is`, `Vermijd cynisme of ironie | Maak het niet zwaar of dramatisch | Vermijd verzoenende of mitigerende taal | Geen branchejargon overnemen`, `Het kan eerlijker — en we wachten niet langer.`],
  [`innocent`, `magician`, `Hoopvol en verbeeldingsrijk. We geloven dat morgen lichter kan worden dan vandaag, en maken die belofte tastbaar.`, `Spreek in korte, alledaagse zinnen | Houd de boodschap positief en concreet | Schets een levendig beeld van wat mogelijk wordt | Verbind het concrete moment aan een groter verhaal`, `Vermijd cynisme of ironie | Maak het niet zwaar of dramatisch | Vermijd vaagheid of mystificatie zonder substantie | Geen lege "magie"-claims`, `Vandaag een kleine stap, morgen een ander beeld.`],
  [`innocent`, `regular`, `Helder en menselijk. We spreken zoals we zelf willen worden aangesproken: zonder kapsones, zonder afstand.`, `Spreek in korte, alledaagse zinnen | Houd de boodschap positief en concreet | Spreek alledaagse taal — geen poespas | Erken dat je weet hoe het is`, `Vermijd cynisme of ironie | Maak het niet zwaar of dramatisch | Vermijd kapsones of expert-toon | Geen grote woorden of stadium-emotie`, `We zijn er voor jou — gewoon, als buren.`],
  [`innocent`, `lover`, `Warm, eerlijk en met aandacht voor wat mensen voelen. We zien je staan en doen ons best.`, `Spreek in korte, alledaagse zinnen | Houd de boodschap positief en concreet | Spreek de zintuigen aan — beelden, geur, klank | Maak het persoonlijk en op de mens gericht`, `Vermijd cynisme of ironie | Maak het niet zwaar of dramatisch | Vermijd technische of klinische taal | Geen koude functionele lijst`, `We willen dat het hier goed voelt — voor jou.`],
  [`innocent`, `jester`, `Licht, vriendelijk en speels. We nemen het serieus zonder het zwaar te maken.`, `Spreek in korte, alledaagse zinnen | Houd de boodschap positief en concreet | Gebruik humor om de waarheid te zeggen | Hou de toon licht en menselijk`, `Vermijd cynisme of ironie | Maak het niet zwaar of dramatisch | Vermijd cynisme of leedvermaak | Geen grappen ten koste van anderen`, `Het mag ook gewoon leuk zijn — en dat is precies wat we doen.`],
  [`innocent`, `caregiver`, `Warm, geruststellend en eerlijk. We staan voor het goede en zorgen dat het ook bij jou aankomt.`, `Spreek in korte, alledaagse zinnen | Houd de boodschap positief en concreet | Toon begrip voordat je een oplossing biedt | Spreek warm en geruststellend`, `Vermijd cynisme of ironie | Maak het niet zwaar of dramatisch | Vermijd betuttelen of paternalistisch klinken | Geen "we zorgen voor onze..." (mensen zijn niet van je)`, `We zorgen ervoor — en houden je op de hoogte.`],
  [`innocent`, `ruler`, `Eerlijk en geordend. We geloven in eenvoud, en zorgen dat alles op zijn plek staat.`, `Spreek in korte, alledaagse zinnen | Houd de boodschap positief en concreet | Spreek met rust en autoriteit | Wees duidelijk over wat de standaard is`, `Vermijd cynisme of ironie | Maak het niet zwaar of dramatisch | Vermijd arrogantie of statusvertoon | Geen onnodig formele of bureaucratische taal`, `Eerlijk geregeld, voor iedereen op dezelfde manier.`],
  [`innocent`, `creator`, `Hoopvol en vindingrijk. We geloven dat morgen mooier kan zijn, en bouwen het zelf.`, `Spreek in korte, alledaagse zinnen | Houd de boodschap positief en concreet | Toon ambacht en oog voor detail | Spreek over wat je bouwt, niet alleen wat je belooft`, `Vermijd cynisme of ironie | Maak het niet zwaar of dramatisch | Vermijd holle "innovatie"-claims | Geen designjargon zonder uitleg`, `We maken iets nieuws — eenvoudig, eerlijk en met aandacht.`],
  [`sage`, `innocent`, `Wijs en helder. We delen wat we weten, maar zo dat iedereen het volgt.`, `Onderbouw uitspraken met feit of bron | Leg de redenering uit, niet alleen de conclusie | Spreek in korte, alledaagse zinnen | Houd de boodschap positief en concreet`, `Vermijd jargon dat alleen experts begrijpen | Niet uit de hoogte spreken of betuttelen | Vermijd cynisme of ironie | Maak het niet zwaar of dramatisch`, `De feiten zijn duidelijk — en we vertellen ze in gewone taal.`],
  [`sage`, `explorer`, `Onderzoekend en nieuwsgierig. We geven kennis als kompas voor wie zelf verder wil.`, `Onderbouw uitspraken met feit of bron | Leg de redenering uit, niet alleen de conclusie | Nodig uit, in plaats van overtuigen | Spreek over mogelijkheden en open horizonten`, `Vermijd jargon dat alleen experts begrijpen | Niet uit de hoogte spreken of betuttelen | Vermijd voorschrijven of geboden | Geen kleinmakende boodschap`, `Hier is wat we weten — wat je ermee doet, bepaal je zelf.`],
  [`sage`, `hero`, `Doortastend en onderbouwd. Zelfverzekerd zonder bluf — beslissingen worden uitgesproken én verantwoord.`, `Onderbouw uitspraken met feit of bron | Leg de redenering uit, niet alleen de conclusie | Spreek in actie-werkwoorden | Wees concreet over wat we gaan doen`, `Vermijd jargon dat alleen experts begrijpen | Niet uit de hoogte spreken of betuttelen | Vermijd onnodige slagen om de arm | Geen marketing-bombarie of holle claims`, `We hebben drie maanden onderzoek gedaan, en weten waar we morgen mee beginnen.`],
  [`sage`, `outlaw`, `Kritisch en weloverwogen. We dagen de heersende kennis uit, met argumenten in plaats van bombarie.`, `Onderbouw uitspraken met feit of bron | Leg de redenering uit, niet alleen de conclusie | Spreek scherp en zonder omhaal | Kies positie, ook als het ongemakkelijk is`, `Vermijd jargon dat alleen experts begrijpen | Niet uit de hoogte spreken of betuttelen | Vermijd verzoenende of mitigerende taal | Geen branchejargon overnemen`, `De aanname klopt niet meer — en hier is waarom.`],
  [`sage`, `magician`, `Visionair en gefundeerd. We zien wat anderen niet zien, en kunnen het ook uitleggen.`, `Onderbouw uitspraken met feit of bron | Leg de redenering uit, niet alleen de conclusie | Schets een levendig beeld van wat mogelijk wordt | Verbind het concrete moment aan een groter verhaal`, `Vermijd jargon dat alleen experts begrijpen | Niet uit de hoogte spreken of betuttelen | Vermijd vaagheid of mystificatie zonder substantie | Geen lege "magie"-claims`, `Wat morgen logisch zal lijken, kunnen we vandaag al onderbouwen.`],
  [`sage`, `regular`, `Toegankelijke expertise. Kennis zonder kapsones, in de taal van de lezer.`, `Onderbouw uitspraken met feit of bron | Leg de redenering uit, niet alleen de conclusie | Spreek alledaagse taal — geen poespas | Erken dat je weet hoe het is`, `Vermijd jargon dat alleen experts begrijpen | Niet uit de hoogte spreken of betuttelen | Vermijd kapsones of expert-toon | Geen grote woorden of stadium-emotie`, `Eindelijk iemand die het uitlegt zoals je het wilt horen.`],
  [`sage`, `lover`, `Geletterd en betrokken. We delen wat we weten met aandacht voor wie het ontvangt.`, `Onderbouw uitspraken met feit of bron | Leg de redenering uit, niet alleen de conclusie | Spreek de zintuigen aan — beelden, geur, klank | Maak het persoonlijk en op de mens gericht`, `Vermijd jargon dat alleen experts begrijpen | Niet uit de hoogte spreken of betuttelen | Vermijd technische of klinische taal | Geen koude functionele lijst`, `We weten waar we het over hebben, en we vertellen het met zorg.`],
  [`sage`, `jester`, `Geestig en intelligent. We brengen kennis met een knipoog — relativering zonder de inhoud te verraden.`, `Onderbouw uitspraken met feit of bron | Leg de redenering uit, niet alleen de conclusie | Gebruik humor om de waarheid te zeggen | Hou de toon licht en menselijk`, `Vermijd jargon dat alleen experts begrijpen | Niet uit de hoogte spreken of betuttelen | Vermijd cynisme of leedvermaak | Geen grappen ten koste van anderen`, `De waarheid is minder spannend dan het verhaal — en wel zo nuttig.`],
  [`sage`, `caregiver`, `Wijs en zorgzaam. We delen kennis op het tempo van de luisteraar.`, `Onderbouw uitspraken met feit of bron | Leg de redenering uit, niet alleen de conclusie | Toon begrip voordat je een oplossing biedt | Spreek warm en geruststellend`, `Vermijd jargon dat alleen experts begrijpen | Niet uit de hoogte spreken of betuttelen | Vermijd betuttelen of paternalistisch klinken | Geen "we zorgen voor onze..." (mensen zijn niet van je)`, `We leggen het zo vaak uit als nodig — er is geen domme vraag.`],
  [`sage`, `ruler`, `Gezaghebbend en zelfverzekerd. Wat we zeggen klopt, en het wordt ook gehoord.`, `Onderbouw uitspraken met feit of bron | Leg de redenering uit, niet alleen de conclusie | Spreek met rust en autoriteit | Wees duidelijk over wat de standaard is`, `Vermijd jargon dat alleen experts begrijpen | Niet uit de hoogte spreken of betuttelen | Vermijd arrogantie of statusvertoon | Geen onnodig formele of bureaucratische taal`, `De richting is helder, en we onderbouwen waarom.`],
  [`sage`, `creator`, `Doordacht en origineel. We bouwen op wat we weten en durven nieuwe wegen te bedenken.`, `Onderbouw uitspraken met feit of bron | Leg de redenering uit, niet alleen de conclusie | Toon ambacht en oog voor detail | Spreek over wat je bouwt, niet alleen wat je belooft`, `Vermijd jargon dat alleen experts begrijpen | Niet uit de hoogte spreken of betuttelen | Vermijd holle "innovatie"-claims | Geen designjargon zonder uitleg`, `We weten waarom dit werkt — en daarom maken we 't anders.`],
  [`explorer`, `innocent`, `Open, hoopvol en uitnodigend. We geloven dat onderweg ontdekt mag worden — eerlijk en zonder druk.`, `Nodig uit, in plaats van overtuigen | Spreek over mogelijkheden en open horizonten | Spreek in korte, alledaagse zinnen | Houd de boodschap positief en concreet`, `Vermijd voorschrijven of geboden | Geen kleinmakende boodschap | Vermijd cynisme of ironie | Maak het niet zwaar of dramatisch`, `Ga je mee kijken? We hebben geen vast antwoord, wel een goede vraag.`],
  [`explorer`, `sage`, `Nieuwsgierig en onderbouwd. We willen weten hoe het zit, en delen wat we onderweg leren.`, `Nodig uit, in plaats van overtuigen | Spreek over mogelijkheden en open horizonten | Onderbouw uitspraken met feit of bron | Leg de redenering uit, niet alleen de conclusie`, `Vermijd voorschrijven of geboden | Geen kleinmakende boodschap | Vermijd jargon dat alleen experts begrijpen | Niet uit de hoogte spreken of betuttelen`, `We onderzoeken het ter plekke — en nemen je mee in wat we vinden.`],
  [`explorer`, `hero`, `Avontuurlijk en doelgericht. Op pad gaan is geen doel op zich — de top wel.`, `Nodig uit, in plaats van overtuigen | Spreek over mogelijkheden en open horizonten | Spreek in actie-werkwoorden | Wees concreet over wat we gaan doen`, `Vermijd voorschrijven of geboden | Geen kleinmakende boodschap | Vermijd onnodige slagen om de arm | Geen marketing-bombarie of holle claims`, `De route ligt voor ons. We beginnen morgen vroeg.`],
  [`explorer`, `outlaw`, `Ongebonden en uitdagend. We gaan waar anderen niet komen, en doen het op onze manier.`, `Nodig uit, in plaats van overtuigen | Spreek over mogelijkheden en open horizonten | Spreek scherp en zonder omhaal | Kies positie, ook als het ongemakkelijk is`, `Vermijd voorschrijven of geboden | Geen kleinmakende boodschap | Vermijd verzoenende of mitigerende taal | Geen branchejargon overnemen`, `Niemand vroeg of het kon — we zijn allang vertrokken.`],
  [`explorer`, `magician`, `Verkennend en visionair. Onderweg ontstaat het beeld van wat mogelijk is.`, `Nodig uit, in plaats van overtuigen | Spreek over mogelijkheden en open horizonten | Schets een levendig beeld van wat mogelijk wordt | Verbind het concrete moment aan een groter verhaal`, `Vermijd voorschrijven of geboden | Geen kleinmakende boodschap | Vermijd vaagheid of mystificatie zonder substantie | Geen lege "magie"-claims`, `We weten nog niet precies wat we vinden — en juist daarom gaan we.`],
  [`explorer`, `regular`, `Avontuurlijk en herkenbaar. Niet exotisch voor de elite — open voor wie mee wil.`, `Nodig uit, in plaats van overtuigen | Spreek over mogelijkheden en open horizonten | Spreek alledaagse taal — geen poespas | Erken dat je weet hoe het is`, `Vermijd voorschrijven of geboden | Geen kleinmakende boodschap | Vermijd kapsones of expert-toon | Geen grote woorden of stadium-emotie`, `Iedereen kan mee, je hoeft geen specialist te zijn.`],
  [`explorer`, `lover`, `Zintuiglijk en open. We willen voelen wat er nog niet beschreven is.`, `Nodig uit, in plaats van overtuigen | Spreek over mogelijkheden en open horizonten | Spreek de zintuigen aan — beelden, geur, klank | Maak het persoonlijk en op de mens gericht`, `Vermijd voorschrijven of geboden | Geen kleinmakende boodschap | Vermijd technische of klinische taal | Geen koude functionele lijst`, `Pas onderweg merk je wat het met je doet.`],
  [`explorer`, `jester`, `Vrolijk en avontuurlijk. We dwalen graag, en lachen om wat we onderweg tegenkomen.`, `Nodig uit, in plaats van overtuigen | Spreek over mogelijkheden en open horizonten | Gebruik humor om de waarheid te zeggen | Hou de toon licht en menselijk`, `Vermijd voorschrijven of geboden | Geen kleinmakende boodschap | Vermijd cynisme of leedvermaak | Geen grappen ten koste van anderen`, `De route loopt anders dan we dachten — en dat is precies leuk.`],
  [`explorer`, `caregiver`, `Uitnodigend en zorgzaam. We gaan op pad, maar laten niemand achter.`, `Nodig uit, in plaats van overtuigen | Spreek over mogelijkheden en open horizonten | Toon begrip voordat je een oplossing biedt | Spreek warm en geruststellend`, `Vermijd voorschrijven of geboden | Geen kleinmakende boodschap | Vermijd betuttelen of paternalistisch klinken | Geen "we zorgen voor onze..." (mensen zijn niet van je)`, `We kijken vooruit én zorgen dat iedereen aanhaakt.`],
  [`explorer`, `ruler`, `Verkennend en geordend. We trekken nieuwe paden, maar zorgen dat ze ook begaanbaar zijn.`, `Nodig uit, in plaats van overtuigen | Spreek over mogelijkheden en open horizonten | Spreek met rust en autoriteit | Wees duidelijk over wat de standaard is`, `Vermijd voorschrijven of geboden | Geen kleinmakende boodschap | Vermijd arrogantie of statusvertoon | Geen onnodig formele of bureaucratische taal`, `We gaan vooraan lopen — én tekenen de kaart voor wie volgt.`],
  [`explorer`, `creator`, `Verkennend en makend. Onderweg ontstaat wat er nog niet was.`, `Nodig uit, in plaats van overtuigen | Spreek over mogelijkheden en open horizonten | Toon ambacht en oog voor detail | Spreek over wat je bouwt, niet alleen wat je belooft`, `Vermijd voorschrijven of geboden | Geen kleinmakende boodschap | Vermijd holle "innovatie"-claims | Geen designjargon zonder uitleg`, `We hebben geen blauwdruk — we bouwen 'm ter plekke.`],
  [`hero`, `innocent`, `Doelgericht en oprecht. We pakken aan, met heldere intenties — geen verborgen agenda.`, `Spreek in actie-werkwoorden | Wees concreet over wat we gaan doen | Spreek in korte, alledaagse zinnen | Houd de boodschap positief en concreet`, `Vermijd onnodige slagen om de arm | Geen marketing-bombarie of holle claims | Vermijd cynisme of ironie | Maak het niet zwaar of dramatisch`, `We zeggen wat we doen, en we doen wat we zeggen.`],
  [`hero`, `sage`, `Doortastend en onderbouwd. Zelfverzekerd zonder bluf — beslissingen worden uitgesproken én verantwoord.`, `Spreek in actie-werkwoorden | Wees concreet over wat we gaan doen | Onderbouw uitspraken met feit of bron | Leg de redenering uit, niet alleen de conclusie`, `Vermijd onnodige slagen om de arm | Geen marketing-bombarie of holle claims | Vermijd jargon dat alleen experts begrijpen | Niet uit de hoogte spreken of betuttelen`, `We hebben de cijfers, we weten wat we doen — en we beginnen vandaag.`],
  [`hero`, `explorer`, `Krachtig en avontuurlijk. We pakken aan én verkennen — geen comfort zone als excuus.`, `Spreek in actie-werkwoorden | Wees concreet over wat we gaan doen | Nodig uit, in plaats van overtuigen | Spreek over mogelijkheden en open horizonten`, `Vermijd onnodige slagen om de arm | Geen marketing-bombarie of holle claims | Vermijd voorschrijven of geboden | Geen kleinmakende boodschap`, `We weten waar we naartoe willen. De route vinden we onderweg.`],
  [`hero`, `outlaw`, `Strijdvaardig en eigenzinnig. We winnen door anders te zijn, niet door harder te schreeuwen.`, `Spreek in actie-werkwoorden | Wees concreet over wat we gaan doen | Spreek scherp en zonder omhaal | Kies positie, ook als het ongemakkelijk is`, `Vermijd onnodige slagen om de arm | Geen marketing-bombarie of holle claims | Vermijd verzoenende of mitigerende taal | Geen branchejargon overnemen`, `Iedereen denkt dat het zo moet — wij bewijzen dat het anders kan.`],
  [`hero`, `magician`, `Intens en doelgericht. We dromen groot en handelen klein, dag voor dag.`, `Spreek in actie-werkwoorden | Wees concreet over wat we gaan doen | Schets een levendig beeld van wat mogelijk wordt | Verbind het concrete moment aan een groter verhaal`, `Vermijd onnodige slagen om de arm | Geen marketing-bombarie of holle claims | Vermijd vaagheid of mystificatie zonder substantie | Geen lege "magie"-claims`, `Wat onmogelijk leek begint vandaag — met de eerste concrete stap.`],
  [`hero`, `regular`, `Doelgericht en menselijk. We pakken aan, zonder ons boven mensen te plaatsen.`, `Spreek in actie-werkwoorden | Wees concreet over wat we gaan doen | Spreek alledaagse taal — geen poespas | Erken dat je weet hoe het is`, `Vermijd onnodige slagen om de arm | Geen marketing-bombarie of holle claims | Vermijd kapsones of expert-toon | Geen grote woorden of stadium-emotie`, `We zetten 'm voor je op de rails — gewoon, zoals afgesproken.`],
  [`hero`, `lover`, `Doelgericht en bewust. We pakken aan, met aandacht voor wie het raakt.`, `Spreek in actie-werkwoorden | Wees concreet over wat we gaan doen | Spreek de zintuigen aan — beelden, geur, klank | Maak het persoonlijk en op de mens gericht`, `Vermijd onnodige slagen om de arm | Geen marketing-bombarie of holle claims | Vermijd technische of klinische taal | Geen koude functionele lijst`, `We doen het goed, en met smaak.`],
  [`hero`, `jester`, `Krachtig met een knipoog. We pakken aan, maar nemen onszelf niet te serieus.`, `Spreek in actie-werkwoorden | Wees concreet over wat we gaan doen | Gebruik humor om de waarheid te zeggen | Hou de toon licht en menselijk`, `Vermijd onnodige slagen om de arm | Geen marketing-bombarie of holle claims | Vermijd cynisme of leedvermaak | Geen grappen ten koste van anderen`, `Mouwen omhoog — en eerst even lachen om gisteren.`],
  [`hero`, `caregiver`, `Doortastend en zorgvuldig. We doen wat nodig is, maar laten niemand vallen.`, `Spreek in actie-werkwoorden | Wees concreet over wat we gaan doen | Toon begrip voordat je een oplossing biedt | Spreek warm en geruststellend`, `Vermijd onnodige slagen om de arm | Geen marketing-bombarie of holle claims | Vermijd betuttelen of paternalistisch klinken | Geen "we zorgen voor onze..." (mensen zijn niet van je)`, `We pakken het aan — en houden ondertussen iedereen aangehaakt.`],
  [`hero`, `ruler`, `Doelgericht en gezaghebbend. We presteren, en we leiden anderen mee.`, `Spreek in actie-werkwoorden | Wees concreet over wat we gaan doen | Spreek met rust en autoriteit | Wees duidelijk over wat de standaard is`, `Vermijd onnodige slagen om de arm | Geen marketing-bombarie of holle claims | Vermijd arrogantie of statusvertoon | Geen onnodig formele of bureaucratische taal`, `Wij zetten de standaard — en handhaven 'm.`],
  [`hero`, `creator`, `Krachtig en vindingrijk. We pakken aan, en we maken iets wat er nog niet was.`, `Spreek in actie-werkwoorden | Wees concreet over wat we gaan doen | Toon ambacht en oog voor detail | Spreek over wat je bouwt, niet alleen wat je belooft`, `Vermijd onnodige slagen om de arm | Geen marketing-bombarie of holle claims | Vermijd holle "innovatie"-claims | Geen designjargon zonder uitleg`, `We bouwen wat we zelf nodig hadden — en gaan ermee aan de slag.`],
  [`outlaw`, `innocent`, `Eigenzinnig en eerlijk. We geloven in het goede, en wachten niet op wie ons in de weg staat.`, `Spreek scherp en zonder omhaal | Kies positie, ook als het ongemakkelijk is | Spreek in korte, alledaagse zinnen | Houd de boodschap positief en concreet`, `Vermijd verzoenende of mitigerende taal | Geen branchejargon overnemen | Vermijd cynisme of ironie | Maak het niet zwaar of dramatisch`, `Het mag eerlijker — en daar maken we vandaag werk van.`],
  [`outlaw`, `sage`, `Kritisch en onderbouwd. We dagen het bekende uit, met argumenten in plaats van geluid.`, `Spreek scherp en zonder omhaal | Kies positie, ook als het ongemakkelijk is | Onderbouw uitspraken met feit of bron | Leg de redenering uit, niet alleen de conclusie`, `Vermijd verzoenende of mitigerende taal | Geen branchejargon overnemen | Vermijd jargon dat alleen experts begrijpen | Niet uit de hoogte spreken of betuttelen`, `De aanname klopt niet meer — en dit is waarom.`],
  [`outlaw`, `explorer`, `Ongebonden en uitdagend. We gaan waar het niet hoort, en doen het op onze manier.`, `Spreek scherp en zonder omhaal | Kies positie, ook als het ongemakkelijk is | Nodig uit, in plaats van overtuigen | Spreek over mogelijkheden en open horizonten`, `Vermijd verzoenende of mitigerende taal | Geen branchejargon overnemen | Vermijd voorschrijven of geboden | Geen kleinmakende boodschap`, `Geen toestemming gevraagd — wel resultaat geboekt.`],
  [`outlaw`, `hero`, `Strijdvaardig en doelgericht. We winnen door anders te durven, niet door volgzaamheid.`, `Spreek scherp en zonder omhaal | Kies positie, ook als het ongemakkelijk is | Spreek in actie-werkwoorden | Wees concreet over wat we gaan doen`, `Vermijd verzoenende of mitigerende taal | Geen branchejargon overnemen | Vermijd onnodige slagen om de arm | Geen marketing-bombarie of holle claims`, `De regels werkten niet voor ons — dus schreven we nieuwe.`],
  [`outlaw`, `magician`, `Disruptief en visionair. We breken af wat klem zat, en bouwen wat morgen normaal wordt.`, `Spreek scherp en zonder omhaal | Kies positie, ook als het ongemakkelijk is | Schets een levendig beeld van wat mogelijk wordt | Verbind het concrete moment aan een groter verhaal`, `Vermijd verzoenende of mitigerende taal | Geen branchejargon overnemen | Vermijd vaagheid of mystificatie zonder substantie | Geen lege "magie"-claims`, `Wat een waanidee leek vorig jaar, is dit jaar standaard.`],
  [`outlaw`, `regular`, `Eigenzinnig en herkenbaar. We zijn radicaal voor mensen die zich miskend voelen — niet voor de show.`, `Spreek scherp en zonder omhaal | Kies positie, ook als het ongemakkelijk is | Spreek alledaagse taal — geen poespas | Erken dat je weet hoe het is`, `Vermijd verzoenende of mitigerende taal | Geen branchejargon overnemen | Vermijd kapsones of expert-toon | Geen grote woorden of stadium-emotie`, `Iedereen voelde 't, niemand zei 't. Wij wel.`],
  [`outlaw`, `lover`, `Brutaal en zintuiglijk. We dagen smaak uit, met genot als wapen.`, `Spreek scherp en zonder omhaal | Kies positie, ook als het ongemakkelijk is | Spreek de zintuigen aan — beelden, geur, klank | Maak het persoonlijk en op de mens gericht`, `Vermijd verzoenende of mitigerende taal | Geen branchejargon overnemen | Vermijd technische of klinische taal | Geen koude functionele lijst`, `Iedereen serveert vanille. Wij gooien er chili in.`],
  [`outlaw`, `jester`, `Brutaal en humoristisch. We dagen het systeem uit — met een grijns die het serieus maakt.`, `Spreek scherp en zonder omhaal | Kies positie, ook als het ongemakkelijk is | Gebruik humor om de waarheid te zeggen | Hou de toon licht en menselijk`, `Vermijd verzoenende of mitigerende taal | Geen branchejargon overnemen | Vermijd cynisme of leedvermaak | Geen grappen ten koste van anderen`, `Bewaar je verontwaardiging — we hebben pas net door wat hier mis is.`],
  [`outlaw`, `caregiver`, `Eigenzinnig en beschermend. We breken regels voor mensen die het anders niet redden.`, `Spreek scherp en zonder omhaal | Kies positie, ook als het ongemakkelijk is | Toon begrip voordat je een oplossing biedt | Spreek warm en geruststellend`, `Vermijd verzoenende of mitigerende taal | Geen branchejargon overnemen | Vermijd betuttelen of paternalistisch klinken | Geen "we zorgen voor onze..." (mensen zijn niet van je)`, `Wat onrechtvaardig was, hebben we vandaag aangepakt.`],
  [`outlaw`, `ruler`, `Disruptief en sturend. We bouwen het nieuwe regime — en houden 't strak.`, `Spreek scherp en zonder omhaal | Kies positie, ook als het ongemakkelijk is | Spreek met rust en autoriteit | Wees duidelijk over wat de standaard is`, `Vermijd verzoenende of mitigerende taal | Geen branchejargon overnemen | Vermijd arrogantie of statusvertoon | Geen onnodig formele of bureaucratische taal`, `De oude orde was traag. Wij gaan vóór ze door.`],
  [`outlaw`, `creator`, `Rebels en vindingrijk. We maken wat er niet was, en accepteren geen "zo werkt het hier".`, `Spreek scherp en zonder omhaal | Kies positie, ook als het ongemakkelijk is | Toon ambacht en oog voor detail | Spreek over wat je bouwt, niet alleen wat je belooft`, `Vermijd verzoenende of mitigerende taal | Geen branchejargon overnemen | Vermijd holle "innovatie"-claims | Geen designjargon zonder uitleg`, `Niemand vroeg erom — dus moest het er wel komen.`],
  [`magician`, `innocent`, `Hoopvol en verbeeldingsrijk. We laten zien dat morgen lichter kan worden — en hoe we dat doen.`, `Schets een levendig beeld van wat mogelijk wordt | Verbind het concrete moment aan een groter verhaal | Spreek in korte, alledaagse zinnen | Houd de boodschap positief en concreet`, `Vermijd vaagheid of mystificatie zonder substantie | Geen lege "magie"-claims | Vermijd cynisme of ironie | Maak het niet zwaar of dramatisch`, `Een kleine ingreep vandaag, een ander beeld over een jaar.`],
  [`magician`, `sage`, `Visionair en gefundeerd. We zien wat anderen niet zien, en kunnen het ook onderbouwen.`, `Schets een levendig beeld van wat mogelijk wordt | Verbind het concrete moment aan een groter verhaal | Onderbouw uitspraken met feit of bron | Leg de redenering uit, niet alleen de conclusie`, `Vermijd vaagheid of mystificatie zonder substantie | Geen lege "magie"-claims | Vermijd jargon dat alleen experts begrijpen | Niet uit de hoogte spreken of betuttelen`, `Wat morgen logisch zal lijken, leggen we vandaag al uit.`],
  [`magician`, `explorer`, `Verkennend en visionair. Onderweg ontvouwt zich wat mogelijk is.`, `Schets een levendig beeld van wat mogelijk wordt | Verbind het concrete moment aan een groter verhaal | Nodig uit, in plaats van overtuigen | Spreek over mogelijkheden en open horizonten`, `Vermijd vaagheid of mystificatie zonder substantie | Geen lege "magie"-claims | Vermijd voorschrijven of geboden | Geen kleinmakende boodschap`, `We weten nog niet precies wat we vinden — en dat is precies wat 't waardevol maakt.`],
  [`magician`, `hero`, `Doelgericht en transformerend. We dromen groot, en bewegen vandaag al naar het nieuwe.`, `Schets een levendig beeld van wat mogelijk wordt | Verbind het concrete moment aan een groter verhaal | Spreek in actie-werkwoorden | Wees concreet over wat we gaan doen`, `Vermijd vaagheid of mystificatie zonder substantie | Geen lege "magie"-claims | Vermijd onnodige slagen om de arm | Geen marketing-bombarie of holle claims`, `Wat onmogelijk leek, begint vandaag met deze ene stap.`],
  [`magician`, `outlaw`, `Disruptief en visionair. We breken af wat klem zit, en bouwen wat morgen normaal wordt.`, `Schets een levendig beeld van wat mogelijk wordt | Verbind het concrete moment aan een groter verhaal | Spreek scherp en zonder omhaal | Kies positie, ook als het ongemakkelijk is`, `Vermijd vaagheid of mystificatie zonder substantie | Geen lege "magie"-claims | Vermijd verzoenende of mitigerende taal | Geen branchejargon overnemen`, `Wat een waanidee leek vorig jaar, is dit jaar standaard.`],
  [`magician`, `regular`, `Verbeeldingsrijk en toegankelijk. We dromen groot, in gewone taal.`, `Schets een levendig beeld van wat mogelijk wordt | Verbind het concrete moment aan een groter verhaal | Spreek alledaagse taal — geen poespas | Erken dat je weet hoe het is`, `Vermijd vaagheid of mystificatie zonder substantie | Geen lege "magie"-claims | Vermijd kapsones of expert-toon | Geen grote woorden of stadium-emotie`, `Stel je voor dat het anders kan — en wat dat voor jou betekent.`],
  [`magician`, `lover`, `Sprookjesachtig en zintuiglijk. We bouwen ervaringen die mensen raken.`, `Schets een levendig beeld van wat mogelijk wordt | Verbind het concrete moment aan een groter verhaal | Spreek de zintuigen aan — beelden, geur, klank | Maak het persoonlijk en op de mens gericht`, `Vermijd vaagheid of mystificatie zonder substantie | Geen lege "magie"-claims | Vermijd technische of klinische taal | Geen koude functionele lijst`, `Even niet meer denken — alleen voelen wat hier gebeurt.`],
  [`magician`, `jester`, `Verbeeldingsrijk en speels. We maken het onmogelijke licht — niet plechtig.`, `Schets een levendig beeld van wat mogelijk wordt | Verbind het concrete moment aan een groter verhaal | Gebruik humor om de waarheid te zeggen | Hou de toon licht en menselijk`, `Vermijd vaagheid of mystificatie zonder substantie | Geen lege "magie"-claims | Vermijd cynisme of leedvermaak | Geen grappen ten koste van anderen`, `Klinkt absurd. Werkt 't ook. Probeer maar.`],
  [`magician`, `caregiver`, `Transformerend en zorgzaam. We willen het leven mooier maken, één persoon tegelijk.`, `Schets een levendig beeld van wat mogelijk wordt | Verbind het concrete moment aan een groter verhaal | Toon begrip voordat je een oplossing biedt | Spreek warm en geruststellend`, `Vermijd vaagheid of mystificatie zonder substantie | Geen lege "magie"-claims | Vermijd betuttelen of paternalistisch klinken | Geen "we zorgen voor onze..." (mensen zijn niet van je)`, `Voor jou maakt het 't verschil — en dat is waarom het mag.`],
  [`magician`, `ruler`, `Visionair en sturend. We weten waar het heen gaat, en we leiden de transitie.`, `Schets een levendig beeld van wat mogelijk wordt | Verbind het concrete moment aan een groter verhaal | Spreek met rust en autoriteit | Wees duidelijk over wat de standaard is`, `Vermijd vaagheid of mystificatie zonder substantie | Geen lege "magie"-claims | Vermijd arrogantie of statusvertoon | Geen onnodig formele of bureaucratische taal`, `Dit wordt de standaard — en wij zorgen dat 't ook gebeurt.`],
  [`magician`, `creator`, `Visionair en makend. Wat we zien, bouwen we ook.`, `Schets een levendig beeld van wat mogelijk wordt | Verbind het concrete moment aan een groter verhaal | Toon ambacht en oog voor detail | Spreek over wat je bouwt, niet alleen wat je belooft`, `Vermijd vaagheid of mystificatie zonder substantie | Geen lege "magie"-claims | Vermijd holle "innovatie"-claims | Geen designjargon zonder uitleg`, `Tussen wat we dromen en wat er staat, zit één blauwdruk.`],
  [`regular`, `innocent`, `Helder en menselijk. We doen normaal, en geloven dat 't ergens om gaat.`, `Spreek alledaagse taal — geen poespas | Erken dat je weet hoe het is | Spreek in korte, alledaagse zinnen | Houd de boodschap positief en concreet`, `Vermijd kapsones of expert-toon | Geen grote woorden of stadium-emotie | Vermijd cynisme of ironie | Maak het niet zwaar of dramatisch`, `Niets bijzonders — gewoon het juiste, voor wie 't nodig heeft.`],
  [`regular`, `sage`, `Toegankelijke expertise. Kennis zonder kapsones, in de taal van de lezer.`, `Spreek alledaagse taal — geen poespas | Erken dat je weet hoe het is | Onderbouw uitspraken met feit of bron | Leg de redenering uit, niet alleen de conclusie`, `Vermijd kapsones of expert-toon | Geen grote woorden of stadium-emotie | Vermijd jargon dat alleen experts begrijpen | Niet uit de hoogte spreken of betuttelen`, `Het zit anders dan je dacht — en hier is hoe.`],
  [`regular`, `explorer`, `Herkenbaar en avontuurlijk. Voor wie 't niet exclusief wil houden.`, `Spreek alledaagse taal — geen poespas | Erken dat je weet hoe het is | Nodig uit, in plaats van overtuigen | Spreek over mogelijkheden en open horizonten`, `Vermijd kapsones of expert-toon | Geen grote woorden of stadium-emotie | Vermijd voorschrijven of geboden | Geen kleinmakende boodschap`, `Je hoeft geen specialist te zijn — gewoon zin om mee te gaan.`],
  [`regular`, `hero`, `Gewoon en doortastend. We doen wat we beloofden, en zonder borstgeklop.`, `Spreek alledaagse taal — geen poespas | Erken dat je weet hoe het is | Spreek in actie-werkwoorden | Wees concreet over wat we gaan doen`, `Vermijd kapsones of expert-toon | Geen grote woorden of stadium-emotie | Vermijd onnodige slagen om de arm | Geen marketing-bombarie of holle claims`, `We hebben 't geregeld — net zoals afgesproken.`],
  [`regular`, `outlaw`, `Herkenbaar en eigenzinnig. We spreken namens wie zich miskend voelt — zonder pose.`, `Spreek alledaagse taal — geen poespas | Erken dat je weet hoe het is | Spreek scherp en zonder omhaal | Kies positie, ook als het ongemakkelijk is`, `Vermijd kapsones of expert-toon | Geen grote woorden of stadium-emotie | Vermijd verzoenende of mitigerende taal | Geen branchejargon overnemen`, `Iedereen voelde 't, niemand zei 't. Wij wel.`],
  [`regular`, `magician`, `Toegankelijk en verbeeldingsrijk. Grote ideeën in gewone woorden.`, `Spreek alledaagse taal — geen poespas | Erken dat je weet hoe het is | Schets een levendig beeld van wat mogelijk wordt | Verbind het concrete moment aan een groter verhaal`, `Vermijd kapsones of expert-toon | Geen grote woorden of stadium-emotie | Vermijd vaagheid of mystificatie zonder substantie | Geen lege "magie"-claims`, `Stel je voor dat het anders kan — en wat dat zou schelen.`],
  [`regular`, `lover`, `Warm en herkenbaar. We zien je staan, en je hoeft je niet anders voor te doen.`, `Spreek alledaagse taal — geen poespas | Erken dat je weet hoe het is | Spreek de zintuigen aan — beelden, geur, klank | Maak het persoonlijk en op de mens gericht`, `Vermijd kapsones of expert-toon | Geen grote woorden of stadium-emotie | Vermijd technische of klinische taal | Geen koude functionele lijst`, `Hier mag je gewoon zijn wie je bent.`],
  [`regular`, `jester`, `Herkenbaar en luchtig. Lekker normaal, en niet zonder gevoel voor humor.`, `Spreek alledaagse taal — geen poespas | Erken dat je weet hoe het is | Gebruik humor om de waarheid te zeggen | Hou de toon licht en menselijk`, `Vermijd kapsones of expert-toon | Geen grote woorden of stadium-emotie | Vermijd cynisme of leedvermaak | Geen grappen ten koste van anderen`, `We doen ons werk goed — en lachen om de rest.`],
  [`regular`, `caregiver`, `Herkenbaar en zorgzaam. Naast de mensen, niet erboven.`, `Spreek alledaagse taal — geen poespas | Erken dat je weet hoe het is | Toon begrip voordat je een oplossing biedt | Spreek warm en geruststellend`, `Vermijd kapsones of expert-toon | Geen grote woorden of stadium-emotie | Vermijd betuttelen of paternalistisch klinken | Geen "we zorgen voor onze..." (mensen zijn niet van je)`, `Even gebeld — je hoeft het niet alleen op te lossen.`],
  [`regular`, `ruler`, `Toegankelijk en geordend. We weten hoe 't moet, en we doen 't voor iedereen hetzelfde.`, `Spreek alledaagse taal — geen poespas | Erken dat je weet hoe het is | Spreek met rust en autoriteit | Wees duidelijk over wat de standaard is`, `Vermijd kapsones of expert-toon | Geen grote woorden of stadium-emotie | Vermijd arrogantie of statusvertoon | Geen onnodig formele of bureaucratische taal`, `Helder geregeld — voor iedereen op dezelfde manier.`],
  [`regular`, `creator`, `Herkenbaar en vindingrijk. We maken iets wat past, niet wat indruk maakt.`, `Spreek alledaagse taal — geen poespas | Erken dat je weet hoe het is | Toon ambacht en oog voor detail | Spreek over wat je bouwt, niet alleen wat je belooft`, `Vermijd kapsones of expert-toon | Geen grote woorden of stadium-emotie | Vermijd holle "innovatie"-claims | Geen designjargon zonder uitleg`, `We hebben 't gebouwd zoals we 't zelf zouden willen gebruiken.`],
  [`lover`, `innocent`, `Warm en oprecht. We hebben aandacht voor wat mensen voelen, zonder dat er iets aan vast hoeft te zitten.`, `Spreek de zintuigen aan — beelden, geur, klank | Maak het persoonlijk en op de mens gericht | Spreek in korte, alledaagse zinnen | Houd de boodschap positief en concreet`, `Vermijd technische of klinische taal | Geen koude functionele lijst | Vermijd cynisme of ironie | Maak het niet zwaar of dramatisch`, `Een moment voor jezelf — meer hoeft 't niet te zijn.`],
  [`lover`, `sage`, `Geletterd en betrokken. Esthetiek én substantie, voor wie smaak met denken combineert.`, `Spreek de zintuigen aan — beelden, geur, klank | Maak het persoonlijk en op de mens gericht | Onderbouw uitspraken met feit of bron | Leg de redenering uit, niet alleen de conclusie`, `Vermijd technische of klinische taal | Geen koude functionele lijst | Vermijd jargon dat alleen experts begrijpen | Niet uit de hoogte spreken of betuttelen`, `Het zit in de details — en daar hebben we lang naar gekeken.`],
  [`lover`, `explorer`, `Zintuiglijk en open. Pas onderweg merk je wat het met je doet.`, `Spreek de zintuigen aan — beelden, geur, klank | Maak het persoonlijk en op de mens gericht | Nodig uit, in plaats van overtuigen | Spreek over mogelijkheden en open horizonten`, `Vermijd technische of klinische taal | Geen koude functionele lijst | Vermijd voorschrijven of geboden | Geen kleinmakende boodschap`, `Niet bedacht voor de foto — bedacht voor wie 't beleeft.`],
  [`lover`, `hero`, `Bewust en doelgericht. We willen het goed doen, en met smaak.`, `Spreek de zintuigen aan — beelden, geur, klank | Maak het persoonlijk en op de mens gericht | Spreek in actie-werkwoorden | Wees concreet over wat we gaan doen`, `Vermijd technische of klinische taal | Geen koude functionele lijst | Vermijd onnodige slagen om de arm | Geen marketing-bombarie of holle claims`, `Strak gemaakt — en goed gevoel om bij stil te staan.`],
  [`lover`, `outlaw`, `Brutaal en zintuiglijk. We dagen smaak uit — genot als positie.`, `Spreek de zintuigen aan — beelden, geur, klank | Maak het persoonlijk en op de mens gericht | Spreek scherp en zonder omhaal | Kies positie, ook als het ongemakkelijk is`, `Vermijd technische of klinische taal | Geen koude functionele lijst | Vermijd verzoenende of mitigerende taal | Geen branchejargon overnemen`, `Iedereen serveert vanille. Wij gooien er chili in.`],
  [`lover`, `magician`, `Sprookjesachtig en zintuiglijk. We bouwen momenten die mensen raken.`, `Spreek de zintuigen aan — beelden, geur, klank | Maak het persoonlijk en op de mens gericht | Schets een levendig beeld van wat mogelijk wordt | Verbind het concrete moment aan een groter verhaal`, `Vermijd technische of klinische taal | Geen koude functionele lijst | Vermijd vaagheid of mystificatie zonder substantie | Geen lege "magie"-claims`, `Even niet meer denken — alleen voelen wat hier gebeurt.`],
  [`lover`, `regular`, `Warm en herkenbaar. Mooi, maar niet voor de elite — voor wie 't gewoon waardeert.`, `Spreek de zintuigen aan — beelden, geur, klank | Maak het persoonlijk en op de mens gericht | Spreek alledaagse taal — geen poespas | Erken dat je weet hoe het is`, `Vermijd technische of klinische taal | Geen koude functionele lijst | Vermijd kapsones of expert-toon | Geen grote woorden of stadium-emotie`, `Mooi mag ook gewoon zijn.`],
  [`lover`, `jester`, `Speels en zintuiglijk. We genieten, en lachen er ook bij.`, `Spreek de zintuigen aan — beelden, geur, klank | Maak het persoonlijk en op de mens gericht | Gebruik humor om de waarheid te zeggen | Hou de toon licht en menselijk`, `Vermijd technische of klinische taal | Geen koude functionele lijst | Vermijd cynisme of leedvermaak | Geen grappen ten koste van anderen`, `Pittig, lekker en niet zonder grap.`],
  [`lover`, `caregiver`, `Warm en zorgzaam. We willen dat 't goed voelt voor wie 't ontvangt.`, `Spreek de zintuigen aan — beelden, geur, klank | Maak het persoonlijk en op de mens gericht | Toon begrip voordat je een oplossing biedt | Spreek warm en geruststellend`, `Vermijd technische of klinische taal | Geen koude functionele lijst | Vermijd betuttelen of paternalistisch klinken | Geen "we zorgen voor onze..." (mensen zijn niet van je)`, `Met aandacht gemaakt — voor jou, voor nu.`],
  [`lover`, `ruler`, `Statig en zintuiglijk. Kwaliteit als geboorterecht, smaak als standaard.`, `Spreek de zintuigen aan — beelden, geur, klank | Maak het persoonlijk en op de mens gericht | Spreek met rust en autoriteit | Wees duidelijk over wat de standaard is`, `Vermijd technische of klinische taal | Geen koude functionele lijst | Vermijd arrogantie of statusvertoon | Geen onnodig formele of bureaucratische taal`, `Wie 't herkent, weet 't meteen.`],
  [`lover`, `creator`, `Esthetisch en makend. Vorm is geen extra — vorm is de boodschap.`, `Spreek de zintuigen aan — beelden, geur, klank | Maak het persoonlijk en op de mens gericht | Toon ambacht en oog voor detail | Spreek over wat je bouwt, niet alleen wat je belooft`, `Vermijd technische of klinische taal | Geen koude functionele lijst | Vermijd holle "innovatie"-claims | Geen designjargon zonder uitleg`, `Eerst de schoonheid, dan de functie.`],
  [`jester`, `innocent`, `Speels en vriendelijk. Het leven mag licht zijn, zonder dat we erom hoeven te lachen.`, `Gebruik humor om de waarheid te zeggen | Hou de toon licht en menselijk | Spreek in korte, alledaagse zinnen | Houd de boodschap positief en concreet`, `Vermijd cynisme of leedvermaak | Geen grappen ten koste van anderen | Vermijd cynisme of ironie | Maak het niet zwaar of dramatisch`, `Het mag ook gewoon leuk zijn — en zo bedoelen we het.`],
  [`jester`, `sage`, `Geestig en intelligent. Kennis met een knipoog, relativering zonder de inhoud te verraden.`, `Gebruik humor om de waarheid te zeggen | Hou de toon licht en menselijk | Onderbouw uitspraken met feit of bron | Leg de redenering uit, niet alleen de conclusie`, `Vermijd cynisme of leedvermaak | Geen grappen ten koste van anderen | Vermijd jargon dat alleen experts begrijpen | Niet uit de hoogte spreken of betuttelen`, `De waarheid is minder spannend dan het verhaal — en wel zo nuttig.`],
  [`jester`, `explorer`, `Vrolijk en avontuurlijk. We dwalen graag, en lachen om wat we onderweg meemaken.`, `Gebruik humor om de waarheid te zeggen | Hou de toon licht en menselijk | Nodig uit, in plaats van overtuigen | Spreek over mogelijkheden en open horizonten`, `Vermijd cynisme of leedvermaak | Geen grappen ten koste van anderen | Vermijd voorschrijven of geboden | Geen kleinmakende boodschap`, `De route liep anders dan we dachten — en dat is precies leuk.`],
  [`jester`, `hero`, `Krachtig met een knipoog. Doelgericht zonder zichzelf serieus te nemen.`, `Gebruik humor om de waarheid te zeggen | Hou de toon licht en menselijk | Spreek in actie-werkwoorden | Wees concreet over wat we gaan doen`, `Vermijd cynisme of leedvermaak | Geen grappen ten koste van anderen | Vermijd onnodige slagen om de arm | Geen marketing-bombarie of holle claims`, `Mouwen omhoog — en eerst even lachen om gisteren.`],
  [`jester`, `outlaw`, `Brutaal en humoristisch. We dagen het systeem uit met een grijns.`, `Gebruik humor om de waarheid te zeggen | Hou de toon licht en menselijk | Spreek scherp en zonder omhaal | Kies positie, ook als het ongemakkelijk is`, `Vermijd cynisme of leedvermaak | Geen grappen ten koste van anderen | Vermijd verzoenende of mitigerende taal | Geen branchejargon overnemen`, `Bewaar je verontwaardiging — wij hebben pas net door wat hier mis is.`],
  [`jester`, `magician`, `Verbeeldingsrijk en speels. We maken het onmogelijke licht.`, `Gebruik humor om de waarheid te zeggen | Hou de toon licht en menselijk | Schets een levendig beeld van wat mogelijk wordt | Verbind het concrete moment aan een groter verhaal`, `Vermijd cynisme of leedvermaak | Geen grappen ten koste van anderen | Vermijd vaagheid of mystificatie zonder substantie | Geen lege "magie"-claims`, `Klinkt absurd. Werkt 't ook. Probeer maar.`],
  [`jester`, `regular`, `Herkenbaar en luchtig. Lekker normaal, en niet zonder humor.`, `Gebruik humor om de waarheid te zeggen | Hou de toon licht en menselijk | Spreek alledaagse taal — geen poespas | Erken dat je weet hoe het is`, `Vermijd cynisme of leedvermaak | Geen grappen ten koste van anderen | Vermijd kapsones of expert-toon | Geen grote woorden of stadium-emotie`, `We doen ons werk goed — en lachen om de rest.`],
  [`jester`, `lover`, `Speels en zintuiglijk. We genieten van het moment én van de pointe.`, `Gebruik humor om de waarheid te zeggen | Hou de toon licht en menselijk | Spreek de zintuigen aan — beelden, geur, klank | Maak het persoonlijk en op de mens gericht`, `Vermijd cynisme of leedvermaak | Geen grappen ten koste van anderen | Vermijd technische of klinische taal | Geen koude functionele lijst`, `Pittig, lekker en niet zonder grap.`],
  [`jester`, `caregiver`, `Lichtvoetig en zorgzaam. We maken 't makkelijker, en zelfs een beetje leuker.`, `Gebruik humor om de waarheid te zeggen | Hou de toon licht en menselijk | Toon begrip voordat je een oplossing biedt | Spreek warm en geruststellend`, `Vermijd cynisme of leedvermaak | Geen grappen ten koste van anderen | Vermijd betuttelen of paternalistisch klinken | Geen "we zorgen voor onze..." (mensen zijn niet van je)`, `Even niets serieus — dat helpt vaak meer dan nog een uitleg.`],
  [`jester`, `ruler`, `Geestig en gezaghebbend. We weten waar het naartoe gaat, en houden er een knipoog bij.`, `Gebruik humor om de waarheid te zeggen | Hou de toon licht en menselijk | Spreek met rust en autoriteit | Wees duidelijk over wat de standaard is`, `Vermijd cynisme of leedvermaak | Geen grappen ten koste van anderen | Vermijd arrogantie of statusvertoon | Geen onnodig formele of bureaucratische taal`, `Wij zorgen dat 't loopt — en dat 't onderweg niet saai wordt.`],
  [`jester`, `creator`, `Speels en vindingrijk. Maken is ook spelen, en spelen levert vaak het beste op.`, `Gebruik humor om de waarheid te zeggen | Hou de toon licht en menselijk | Toon ambacht en oog voor detail | Spreek over wat je bouwt, niet alleen wat je belooft`, `Vermijd cynisme of leedvermaak | Geen grappen ten koste van anderen | Vermijd holle "innovatie"-claims | Geen designjargon zonder uitleg`, `We hebben er drie versies van — en de gekste werkte 't beste.`],
  [`caregiver`, `innocent`, `Warm, geruststellend en eerlijk. We staan voor het goede, en zorgen dat 't ook aankomt.`, `Toon begrip voordat je een oplossing biedt | Spreek warm en geruststellend | Spreek in korte, alledaagse zinnen | Houd de boodschap positief en concreet`, `Vermijd betuttelen of paternalistisch klinken | Geen "we zorgen voor onze..." (mensen zijn niet van je) | Vermijd cynisme of ironie | Maak het niet zwaar of dramatisch`, `We regelen 't — en houden je op de hoogte.`],
  [`caregiver`, `sage`, `Wijs en zorgzaam. We delen kennis op het tempo van de luisteraar.`, `Toon begrip voordat je een oplossing biedt | Spreek warm en geruststellend | Onderbouw uitspraken met feit of bron | Leg de redenering uit, niet alleen de conclusie`, `Vermijd betuttelen of paternalistisch klinken | Geen "we zorgen voor onze..." (mensen zijn niet van je) | Vermijd jargon dat alleen experts begrijpen | Niet uit de hoogte spreken of betuttelen`, `We leggen het uit zo vaak als nodig — er is geen domme vraag.`],
  [`caregiver`, `explorer`, `Zorgzaam en uitnodigend. We gaan vooruit, maar laten niemand achter.`, `Toon begrip voordat je een oplossing biedt | Spreek warm en geruststellend | Nodig uit, in plaats van overtuigen | Spreek over mogelijkheden en open horizonten`, `Vermijd betuttelen of paternalistisch klinken | Geen "we zorgen voor onze..." (mensen zijn niet van je) | Vermijd voorschrijven of geboden | Geen kleinmakende boodschap`, `We kijken vooruit, en zorgen dat iedereen aanhaakt.`],
  [`caregiver`, `hero`, `Doortastend en zorgvuldig. We doen wat nodig is, en laten niemand vallen.`, `Toon begrip voordat je een oplossing biedt | Spreek warm en geruststellend | Spreek in actie-werkwoorden | Wees concreet over wat we gaan doen`, `Vermijd betuttelen of paternalistisch klinken | Geen "we zorgen voor onze..." (mensen zijn niet van je) | Vermijd onnodige slagen om de arm | Geen marketing-bombarie of holle claims`, `We pakken het aan — en houden ondertussen iedereen aangehaakt.`],
  [`caregiver`, `outlaw`, `Beschermend en eigenzinnig. We breken regels voor wie 't anders niet redt.`, `Toon begrip voordat je een oplossing biedt | Spreek warm en geruststellend | Spreek scherp en zonder omhaal | Kies positie, ook als het ongemakkelijk is`, `Vermijd betuttelen of paternalistisch klinken | Geen "we zorgen voor onze..." (mensen zijn niet van je) | Vermijd verzoenende of mitigerende taal | Geen branchejargon overnemen`, `Wat onrechtvaardig was, hebben we vandaag aangepakt.`],
  [`caregiver`, `magician`, `Zorgzaam en transformerend. We willen het leven mooier maken, één persoon tegelijk.`, `Toon begrip voordat je een oplossing biedt | Spreek warm en geruststellend | Schets een levendig beeld van wat mogelijk wordt | Verbind het concrete moment aan een groter verhaal`, `Vermijd betuttelen of paternalistisch klinken | Geen "we zorgen voor onze..." (mensen zijn niet van je) | Vermijd vaagheid of mystificatie zonder substantie | Geen lege "magie"-claims`, `Voor jou maakt het 't verschil — en daarom mag het.`],
  [`caregiver`, `regular`, `Herkenbaar en zorgzaam. Naast de mensen, niet erboven.`, `Toon begrip voordat je een oplossing biedt | Spreek warm en geruststellend | Spreek alledaagse taal — geen poespas | Erken dat je weet hoe het is`, `Vermijd betuttelen of paternalistisch klinken | Geen "we zorgen voor onze..." (mensen zijn niet van je) | Vermijd kapsones of expert-toon | Geen grote woorden of stadium-emotie`, `Even gebeld — je hoeft het niet alleen op te lossen.`],
  [`caregiver`, `lover`, `Warm en zorgzaam. We willen dat 't goed voelt voor wie 't ontvangt.`, `Toon begrip voordat je een oplossing biedt | Spreek warm en geruststellend | Spreek de zintuigen aan — beelden, geur, klank | Maak het persoonlijk en op de mens gericht`, `Vermijd betuttelen of paternalistisch klinken | Geen "we zorgen voor onze..." (mensen zijn niet van je) | Vermijd technische of klinische taal | Geen koude functionele lijst`, `Met aandacht gemaakt — voor jou, voor nu.`],
  [`caregiver`, `jester`, `Lichtvoetig en zorgzaam. We maken 't makkelijker — en soms een tikje leuker.`, `Toon begrip voordat je een oplossing biedt | Spreek warm en geruststellend | Gebruik humor om de waarheid te zeggen | Hou de toon licht en menselijk`, `Vermijd betuttelen of paternalistisch klinken | Geen "we zorgen voor onze..." (mensen zijn niet van je) | Vermijd cynisme of leedvermaak | Geen grappen ten koste van anderen`, `Even niets zwaar maken — dat helpt vaak meer dan nog een uitleg.`],
  [`caregiver`, `ruler`, `Verantwoordelijk en gezaghebbend. We zorgen, en we organiseren dat 't blijft kloppen.`, `Toon begrip voordat je een oplossing biedt | Spreek warm en geruststellend | Spreek met rust en autoriteit | Wees duidelijk over wat de standaard is`, `Vermijd betuttelen of paternalistisch klinken | Geen "we zorgen voor onze..." (mensen zijn niet van je) | Vermijd arrogantie of statusvertoon | Geen onnodig formele of bureaucratische taal`, `We hebben 't goed geregeld — voor nu, en voor wat komt.`],
  [`caregiver`, `creator`, `Zorgzaam en vindingrijk. We bedenken wat helpt, en maken 't ook.`, `Toon begrip voordat je een oplossing biedt | Spreek warm en geruststellend | Toon ambacht en oog voor detail | Spreek over wat je bouwt, niet alleen wat je belooft`, `Vermijd betuttelen of paternalistisch klinken | Geen "we zorgen voor onze..." (mensen zijn niet van je) | Vermijd holle "innovatie"-claims | Geen designjargon zonder uitleg`, `We hebben iets gebouwd dat voor jou het verschil maakt.`],
  [`ruler`, `innocent`, `Geordend en eerlijk. We zorgen dat alles op zijn plek staat, voor iedereen op dezelfde manier.`, `Spreek met rust en autoriteit | Wees duidelijk over wat de standaard is | Spreek in korte, alledaagse zinnen | Houd de boodschap positief en concreet`, `Vermijd arrogantie of statusvertoon | Geen onnodig formele of bureaucratische taal | Vermijd cynisme of ironie | Maak het niet zwaar of dramatisch`, `Eerlijk geregeld — voor iedereen op dezelfde manier.`],
  [`ruler`, `sage`, `Gezaghebbend en zelfverzekerd. Wat we zeggen klopt, en het wordt ook gehoord.`, `Spreek met rust en autoriteit | Wees duidelijk over wat de standaard is | Onderbouw uitspraken met feit of bron | Leg de redenering uit, niet alleen de conclusie`, `Vermijd arrogantie of statusvertoon | Geen onnodig formele of bureaucratische taal | Vermijd jargon dat alleen experts begrijpen | Niet uit de hoogte spreken of betuttelen`, `De richting is helder — en we onderbouwen waarom.`],
  [`ruler`, `explorer`, `Sturend en verkennend. We trekken paden, en zorgen dat ze begaanbaar zijn.`, `Spreek met rust en autoriteit | Wees duidelijk over wat de standaard is | Nodig uit, in plaats van overtuigen | Spreek over mogelijkheden en open horizonten`, `Vermijd arrogantie of statusvertoon | Geen onnodig formele of bureaucratische taal | Vermijd voorschrijven of geboden | Geen kleinmakende boodschap`, `We lopen vooraan — en tekenen de kaart voor wie volgt.`],
  [`ruler`, `hero`, `Doelgericht en gezaghebbend. We presteren, en leiden anderen mee.`, `Spreek met rust en autoriteit | Wees duidelijk over wat de standaard is | Spreek in actie-werkwoorden | Wees concreet over wat we gaan doen`, `Vermijd arrogantie of statusvertoon | Geen onnodig formele of bureaucratische taal | Vermijd onnodige slagen om de arm | Geen marketing-bombarie of holle claims`, `Wij zetten de standaard — en handhaven 'm.`],
  [`ruler`, `outlaw`, `Sturend en disruptief. We bouwen het nieuwe regime, en houden 't strak.`, `Spreek met rust en autoriteit | Wees duidelijk over wat de standaard is | Spreek scherp en zonder omhaal | Kies positie, ook als het ongemakkelijk is`, `Vermijd arrogantie of statusvertoon | Geen onnodig formele of bureaucratische taal | Vermijd verzoenende of mitigerende taal | Geen branchejargon overnemen`, `De oude orde was traag. Wij gaan vóór ze door.`],
  [`ruler`, `magician`, `Sturend en visionair. We weten waar het heen gaat, en leiden de transitie.`, `Spreek met rust en autoriteit | Wees duidelijk over wat de standaard is | Schets een levendig beeld van wat mogelijk wordt | Verbind het concrete moment aan een groter verhaal`, `Vermijd arrogantie of statusvertoon | Geen onnodig formele of bureaucratische taal | Vermijd vaagheid of mystificatie zonder substantie | Geen lege "magie"-claims`, `Dit wordt de standaard — en wij zorgen dat 't ook gebeurt.`],
  [`ruler`, `regular`, `Geordend en toegankelijk. We weten hoe 't moet, en doen 't voor iedereen hetzelfde.`, `Spreek met rust en autoriteit | Wees duidelijk over wat de standaard is | Spreek alledaagse taal — geen poespas | Erken dat je weet hoe het is`, `Vermijd arrogantie of statusvertoon | Geen onnodig formele of bureaucratische taal | Vermijd kapsones of expert-toon | Geen grote woorden of stadium-emotie`, `Helder geregeld — voor iedereen op dezelfde manier.`],
  [`ruler`, `lover`, `Statig en zintuiglijk. Kwaliteit als geboorterecht, smaak als standaard.`, `Spreek met rust en autoriteit | Wees duidelijk over wat de standaard is | Spreek de zintuigen aan — beelden, geur, klank | Maak het persoonlijk en op de mens gericht`, `Vermijd arrogantie of statusvertoon | Geen onnodig formele of bureaucratische taal | Vermijd technische of klinische taal | Geen koude functionele lijst`, `Wie 't herkent, weet 't meteen.`],
  [`ruler`, `jester`, `Gezaghebbend en geestig. We weten waar het naartoe gaat, en houden er een knipoog bij.`, `Spreek met rust en autoriteit | Wees duidelijk over wat de standaard is | Gebruik humor om de waarheid te zeggen | Hou de toon licht en menselijk`, `Vermijd arrogantie of statusvertoon | Geen onnodig formele of bureaucratische taal | Vermijd cynisme of leedvermaak | Geen grappen ten koste van anderen`, `Wij zorgen dat 't loopt — én dat 't onderweg niet saai wordt.`],
  [`ruler`, `caregiver`, `Gezaghebbend en verantwoordelijk. We sturen, en we zorgen.`, `Spreek met rust en autoriteit | Wees duidelijk over wat de standaard is | Toon begrip voordat je een oplossing biedt | Spreek warm en geruststellend`, `Vermijd arrogantie of statusvertoon | Geen onnodig formele of bureaucratische taal | Vermijd betuttelen of paternalistisch klinken | Geen "we zorgen voor onze..." (mensen zijn niet van je)`, `We hebben 't goed geregeld — voor nu en voor wat komt.`],
  [`ruler`, `creator`, `Sturend en vindingrijk. We zetten de standaard, én we maken er iets nieuws van.`, `Spreek met rust en autoriteit | Wees duidelijk over wat de standaard is | Toon ambacht en oog voor detail | Spreek over wat je bouwt, niet alleen wat je belooft`, `Vermijd arrogantie of statusvertoon | Geen onnodig formele of bureaucratische taal | Vermijd holle "innovatie"-claims | Geen designjargon zonder uitleg`, `Wij bepalen hoe 't eruit ziet — en zorgen dat 't klopt.`],
  [`creator`, `innocent`, `Hoopvol en vindingrijk. We bouwen aan iets nieuws, eenvoudig en eerlijk.`, `Toon ambacht en oog voor detail | Spreek over wat je bouwt, niet alleen wat je belooft | Spreek in korte, alledaagse zinnen | Houd de boodschap positief en concreet`, `Vermijd holle "innovatie"-claims | Geen designjargon zonder uitleg | Vermijd cynisme of ironie | Maak het niet zwaar of dramatisch`, `We maken iets nieuws — eenvoudig, eerlijk, met aandacht.`],
  [`creator`, `sage`, `Doordacht en origineel. We bouwen op wat we weten, en durven anders.`, `Toon ambacht en oog voor detail | Spreek over wat je bouwt, niet alleen wat je belooft | Onderbouw uitspraken met feit of bron | Leg de redenering uit, niet alleen de conclusie`, `Vermijd holle "innovatie"-claims | Geen designjargon zonder uitleg | Vermijd jargon dat alleen experts begrijpen | Niet uit de hoogte spreken of betuttelen`, `We weten waarom dit werkt — en daarom maken we 't zo.`],
  [`creator`, `explorer`, `Verkennend en makend. Onderweg ontstaat wat er nog niet was.`, `Toon ambacht en oog voor detail | Spreek over wat je bouwt, niet alleen wat je belooft | Nodig uit, in plaats van overtuigen | Spreek over mogelijkheden en open horizonten`, `Vermijd holle "innovatie"-claims | Geen designjargon zonder uitleg | Vermijd voorschrijven of geboden | Geen kleinmakende boodschap`, `We hebben geen blauwdruk — we bouwen 'm ter plekke.`],
  [`creator`, `hero`, `Vindingrijk en doelgericht. We maken wat we zelf nodig hadden, en gaan er nu mee aan de slag.`, `Toon ambacht en oog voor detail | Spreek over wat je bouwt, niet alleen wat je belooft | Spreek in actie-werkwoorden | Wees concreet over wat we gaan doen`, `Vermijd holle "innovatie"-claims | Geen designjargon zonder uitleg | Vermijd onnodige slagen om de arm | Geen marketing-bombarie of holle claims`, `We hebben 't gebouwd — en gaan ermee aan de slag.`],
  [`creator`, `outlaw`, `Rebels en vindingrijk. We maken wat er niet was, en accepteren geen "zo werkt het hier".`, `Toon ambacht en oog voor detail | Spreek over wat je bouwt, niet alleen wat je belooft | Spreek scherp en zonder omhaal | Kies positie, ook als het ongemakkelijk is`, `Vermijd holle "innovatie"-claims | Geen designjargon zonder uitleg | Vermijd verzoenende of mitigerende taal | Geen branchejargon overnemen`, `Niemand vroeg erom — dus moest het er wel komen.`],
  [`creator`, `magician`, `Visionair en makend. Wat we zien, bouwen we ook.`, `Toon ambacht en oog voor detail | Spreek over wat je bouwt, niet alleen wat je belooft | Schets een levendig beeld van wat mogelijk wordt | Verbind het concrete moment aan een groter verhaal`, `Vermijd holle "innovatie"-claims | Geen designjargon zonder uitleg | Vermijd vaagheid of mystificatie zonder substantie | Geen lege "magie"-claims`, `Tussen wat we dromen en wat er staat, zit één blauwdruk.`],
  [`creator`, `regular`, `Vindingrijk en herkenbaar. We maken iets wat past, niet wat indruk maakt.`, `Toon ambacht en oog voor detail | Spreek over wat je bouwt, niet alleen wat je belooft | Spreek alledaagse taal — geen poespas | Erken dat je weet hoe het is`, `Vermijd holle "innovatie"-claims | Geen designjargon zonder uitleg | Vermijd kapsones of expert-toon | Geen grote woorden of stadium-emotie`, `Gebouwd zoals we 't zelf zouden willen gebruiken.`],
  [`creator`, `lover`, `Esthetisch en makend. Vorm is geen extra — vorm is de boodschap.`, `Toon ambacht en oog voor detail | Spreek over wat je bouwt, niet alleen wat je belooft | Spreek de zintuigen aan — beelden, geur, klank | Maak het persoonlijk en op de mens gericht`, `Vermijd holle "innovatie"-claims | Geen designjargon zonder uitleg | Vermijd technische of klinische taal | Geen koude functionele lijst`, `Eerst de schoonheid, dan de functie.`],
  [`creator`, `jester`, `Speels en vindingrijk. Maken is ook spelen, en spelen levert vaak het beste op.`, `Toon ambacht en oog voor detail | Spreek over wat je bouwt, niet alleen wat je belooft | Gebruik humor om de waarheid te zeggen | Hou de toon licht en menselijk`, `Vermijd holle "innovatie"-claims | Geen designjargon zonder uitleg | Vermijd cynisme of leedvermaak | Geen grappen ten koste van anderen`, `Drie versies gemaakt — de gekste werkte 't beste.`],
  [`creator`, `caregiver`, `Vindingrijk en zorgzaam. We bedenken wat helpt, en maken 't ook.`, `Toon ambacht en oog voor detail | Spreek over wat je bouwt, niet alleen wat je belooft | Toon begrip voordat je een oplossing biedt | Spreek warm en geruststellend`, `Vermijd holle "innovatie"-claims | Geen designjargon zonder uitleg | Vermijd betuttelen of paternalistisch klinken | Geen "we zorgen voor onze..." (mensen zijn niet van je)`, `We hebben iets gebouwd dat voor jou het verschil maakt.`],
  [`creator`, `ruler`, `Sturend en vindingrijk. We bepalen de standaard, en bouwen 'm zelf.`, `Toon ambacht en oog voor detail | Spreek over wat je bouwt, niet alleen wat je belooft | Spreek met rust en autoriteit | Wees duidelijk over wat de standaard is`, `Vermijd holle "innovatie"-claims | Geen designjargon zonder uitleg | Vermijd arrogantie of statusvertoon | Geen onnodig formele of bureaucratische taal`, `Wij bepalen hoe 't eruitziet — en zorgen dat 't klopt.`]
];
